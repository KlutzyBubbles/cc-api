	...
	if (isset($body['user']) && isset($body['token'])) {
		...
		if (!$o['valid']) {
			...
			if ($con->hasError()) {
				...
				$con->query("SELECT first_name, middle_initial, last_name, street_no, street_name, suburb, postcode, phone, state FROM users WHERE LOWER(email)=" . $con->quote(strtolower($body['user'])) . " LIMIT 1");
